...
const generateVideo = async (job_id: string, request_data: any) => {
  try {
    ...
    // Step 1: Use Node.js for AI planning (avoid Python template issues)
    const { spawn } = require('child_process');
    const { OpenAI } = require('openai');
    
    // Initialize OpenAI directly in Node.js
    const openai = new (require('openai').OpenAI)({
      apiKey: process.env.OPENAI_API_KEY
    });
    
    // Generate comprehensive plan
    const planningPrompt = `You are a world-class creative director and video strategist. Create a comprehensive, detailed video plan for "${brand_name}".
...
- Call to Action: ${call_to_action || 'Learn more'}
...
Return ONLY valid JSON:
{
  ...
  "technical_specs": {
    ...
    updateJobStatus(job_id, "processing", 25, "Generating detailed creative strategy...");
